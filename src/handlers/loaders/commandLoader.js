...
async function registerCommandsToDiscord(client, clientId, commands, totalSubcommands) {
    if (!clientId) {
    ...
    const guildId = process.env.GUILD_ID || botConfig.guildId;

    if (guildId) {
        logger.info(`Registering ${commandsToRegister.length} commands to Guild (${guildId}) instantly...`);
        await client.rest.put(`/applications/${clientId}/guilds/${guildId}/commands`, { body: commandsToRegister });
        logger.info(`Successfully registered ${commandsToRegister.length} commands to Guild!`);
    } else {
...
export async function registerCommands(client, options = {}) {
    ...
    try {
        ...
        await registerCommandsToDiscord(client, clientId, commands, totalSubcommands);
    } catch (error) {
