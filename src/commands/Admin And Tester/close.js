...
export default {
  ...
  async execute(interaction) {
    ...
    // 2. Ticket Channel Validation
    let isTicket = false;
    if (typeof waitlistService.getTicket === 'function') {
      isTicket = Boolean(waitlistService.getTicket(channel.id));
    } else {
      isTicket = channel.name.startsWith('ticket-');
    }
    ...
    // 3. Remove Ticket from Waitlist Memory
    if (typeof waitlistService.removeTicket === 'function') {
